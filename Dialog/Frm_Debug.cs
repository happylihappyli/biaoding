...
namespace Test1
{
    ...
    public partial class Frm_Debug : Form
    {
        ...
        public void refresh_var()
        {
            ...
            for (var i = 0; i < keys.Count; i++)
            {
                try
                {
                    ...
                    if (pVar.get_obj() != null) item.SubItems.Add(pVar.get_obj().ToString());
                }
                catch (Exception ex)
                {
            ...
            for (var i = 0; i < space.pTrain.get_Vars().Count; i++)
            {
                try
                {
                    ...
                    var item = list_var.Items.Add(pVar.name);
                    item.SubItems.Add(pVar?.get_obj()?.ToString());
                }
                catch (Exception ex)
                {
                    Main.WriteLine(ex.ToString());
        ...
        public void WriteError(string Name,string str_line)
        {
            Action act = delegate ()
            {
                var Item= listView_error.Items.Add(Name);
                Item.SubItems.Add(str_line);
            };
            listView_error.BeginInvoke(act, null);
        }
    }
}
