    ...
    class pcammls_isp_api
    {
        ...
        public static void ConvertYUYV2RGB(uint8_t_ARRAY yuvFrame, uint8_t_ARRAY rgbFrame, int width, int height)
        {
            ...
            for (uint y = 0; y < height; y++)
            {
                ...
                {
                    byte y1 = yuvFrame[(uint)(y * widthstep + x * 2)];
                    byte u = yuvFrame[(uint)(y * widthstep + x * 2) + 1];
                    byte y2 = yuvFrame[(uint)(y * widthstep + x * 2) + 2];
                    byte v = yuvFrame[(uint)(y * widthstep + x * 2) + 3];
                    // R
                    temp = (int)(y1 + (v - 128) * YUV2RGB_CONVERT_MATRIX[0, 2]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x ) + 2] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                    // G
                    temp = (int)(y1 + (u - 128) * YUV2RGB_CONVERT_MATRIX[1, 1] + (v - 128) * YUV2RGB_CONVERT_MATRIX[1, 2]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x ) + 1] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                    // B
                    temp = (int)(y1 + (u - 128) * YUV2RGB_CONVERT_MATRIX[2, 1]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x)] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);

                    // R
                    temp = (int)(y2 + (v - 128) * YUV2RGB_CONVERT_MATRIX[0, 2]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x) + 5] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                    // G
                    temp = (int)(y2 + (u - 128) * YUV2RGB_CONVERT_MATRIX[1, 1] + (v - 128) * YUV2RGB_CONVERT_MATRIX[1, 2]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x) + 4] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                    // B
                    temp = (int)(y2 + (u - 128) * YUV2RGB_CONVERT_MATRIX[2, 1]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x) + 3] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                }
            }
        }
        ...
        public static void ConvertYVYU2RGB(uint8_t_ARRAY yuvFrame, uint8_t_ARRAY rgbFrame, int width, int height)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    byte y1 = yuvFrame[(uint)(y * widthstep + x * 2)];
                    byte v = yuvFrame[(uint)(y * widthstep + x * 2) + 1];
                    byte y2 = yuvFrame[(uint)(y * widthstep + x * 2) + 2];
                    byte u = yuvFrame[(uint)(y * widthstep + x * 2) + 3];
                    // R
                    temp = (int)(y1 + (v - 128) * YUV2RGB_CONVERT_MATRIX[0, 2]);
                    rgbFrame[(uint)(y * width * 3 + 3 * x) + 2] = (byte)(temp < 0 ? 0 : temp > 255 ? 255 : temp);
                    // G
        ...
        static SWIGTYPE_p_unsigned_char IntArrayToSWIGTYPE(int[] array, int len)
        {
            ...
            uint8_t_ARRAY _arr = new uint8_t_ARRAY((uint)size);
            for (uint i = 0; i < size; i++) _arr[i] = bytes[i];
        ...
        static SWIGTYPE_p_unsigned_char IntPtrToSWIGTYPE(IntPtr p)
        {
            ...
            uint8_t_ARRAY _handle = new uint8_t_ARRAY((uint)size);
            for (uint i = 0; i < size; i++) _handle[i] = bytes[i];
        ...
        //    int t = (int)SDK.TY_COMPONENT_RGB_CAM;
