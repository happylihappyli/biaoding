...
namespace Test1
{
    public class C_Camera_hk
    {
        public delegate void Get_Picture(Bitmap picture);

        public Get_Picture call_get_picture ;
        ...
        public void open_device(int index)
        {
            if (m_ltDeviceList.Count == 0 || index == -1)
            {
        ...
        public void ReceiveThreadProcess()
        {
            ...
            while (m_bGrabbing)
            {
                ...
                if (nRet == CErrorDefine.MV_OK && m_pcBitmap!= null)
                {
                    lock (BufForDriverLock)
                    {
                        m_pcImgForDriver = (CImage)pcFrameInfo.Image.Clone();
                    ...
                    if (call_get_picture != null) call_get_picture(m_pcBitmap);// pcFrameInfo.Image);
        ...
        public void set_Exposure_Time(float value)
        {
        ...
        public void DeviceListAcq()
        {
            ...
            for (int i = 0; i < m_ltDeviceList.Count; i++)
            {
                if (m_ltDeviceList[i].nTLayerType == CSystem.MV_GIGE_DEVICE)
                {
                    ...
                    {
                        list_device.Add("GEV: " + gigeInfo.UserDefinedName + " (" + gigeInfo.chSerialNumber + ")");
                    }
                    else
                    {
                        list_device.Add("GEV: " + gigeInfo.chManufacturerName + " " + gigeInfo.chModelName + " (" + gigeInfo.chSerialNumber + ")");
                    }
                }
                else if (m_ltDeviceList[i].nTLayerType == CSystem.MV_USB_DEVICE)
                {
                    ...
                    {
                        list_device.Add("U3V: " + usbInfo.UserDefinedName + " (" + usbInfo.chSerialNumber + ")");
                    }
                    else
                    {
                        list_device.Add("U3V: " + usbInfo.chManufacturerName + " " + usbInfo.chModelName + " (" + usbInfo.chSerialNumber + ")");
                    }
                }
            }
