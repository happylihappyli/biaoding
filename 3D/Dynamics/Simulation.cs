...
namespace JA.Dynamics
{
    ...
    public class Simulation
    {
        ...
        public IEnumerable<BodyState> Integrate(IEnumerable<BodyState> current, double h)
        {
            ...
            BodyState[] next = (BodyState[])current.ToArray().Clone();
        ...
        public RigidBody AddBody(Vector3 center, double mass, VisibleSphere sphere, Pose initialPose, Vector33 initialMotion)
        {
            RigidBody body = new RigidBody(center, mass, sphere, initialPose, initialMotion);
            bodies.Add(body);
            return body;
        }

        public RigidBody AddBody(C_Point3D p_center, double mass, VisibleSphere sphere, Pose initialPose, Vector33 initialMotion)
        {
            Vector3 center = new Vector3(p_center.x, p_center.y, p_center.z);
            RigidBody body = new RigidBody(center, mass, sphere, initialPose, initialMotion);
            bodies.Add(body);
            return body;
        }
        ...
        /// <param name="mesh">物体</param>
        /// <param name="initialPose">初始位置</param>
        /// <param name="initialMotion">初始运动方向，旋转角度</param>
        /// <returns></returns>
        public RigidBody AddBody(
            Vector3 center,
            double mass, VisibleMesh mesh, Pose initialPose, Vector33 initialMotion)
        {
            var body = new RigidBody(center,mass, mesh, initialPose, initialMotion);
            bodies.Add(body);
            return body;
        }
