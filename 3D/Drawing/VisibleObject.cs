    ...
    public abstract class VisibleObject : IRender
    {
        ...
        public void RayTrace(Graphics g, Camera camera, IRayTarget shape, Color color)
        {
            ...
            f.FillColors((i, j) =>
            {
                ...
                var ray = camera.CastRayThroughPixel(new System.Drawing.Point(x, y));
    ...
    public class VisibleMesh : VisibleSolid
    {
        ...
        public override void Render(Graphics g, Camera camera, Pose pose)
        {
            ...
            for (int k = 0; k < Mesh.Faces.Count; k++)
            {
                ...
                //if (k == 2)
                //{
                //    Console.WriteLine(k + "=" + color.Name+ " fill="+fill);
                //}
                g.DrawPath(gp, color, fill);

                //happyli add draw face normal
                if (this.show_normal)
                {
                    g.DrawLine(Pens.Yellow, camera.Project(poly.Nodes[0]), camera.Project(poly.Nodes[0] + poly.Normal));
                }
            }

            //happyli add, camera normal
            //PointF center = camera.Project(new Vector3(0, 0, 0));
            //Point center2 = new Point((int)Math.Round(center.X), (int)Math.Round(center.Y));
            //Ray r = camera.CastRayThroughPixel(center2);
            //Point center3 = new Point((int)Math.Round(center.X) + 10, (int)Math.Round(center.Y));
            //g.DrawLine(Color.Red, center3, camera.Project(r.Direction * 10), 3);//camera.Project(new Vector3(0,0,0))
