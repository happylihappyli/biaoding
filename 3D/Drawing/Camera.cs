...
namespace JA.Drawing
{
    ...
    public class Camera
    {
        ...
        public Camera(string name, Control control, float fov, float sceneSize = 1f)
        {
            ...
            {
                float numberOfTextLinesToMove = ev.Delta * SystemInformation.MouseWheelScrollLines / 120;
                if (numberOfTextLinesToMove > 0)
                {
                    this.SceneSize = this.SceneSize / 1.1f;
                }
                else
                {
                    this.SceneSize = this.SceneSize * 1.1f;
        ...
        {
            if (polygon.Nodes.Length < 3)
            {
                return true;
            }

            PointF center = Project(new Vector3(0, 0, 0));
            Point center2 = new Point((int)Math.Round(center.X), (int)Math.Round(center.Y));
            Ray r = CastRayThroughPixel(center2);
            
            //if (polygon.Name == "g2")
            //{
            //    Console.WriteLine(polygon.Name + "=" + polygon.Normal.ToString());
            //    Console.WriteLine("r=" + r.Direction.ToString());
            //}
            return IsVisible(r.Direction, polygon.Normal); 
        }
