...
namespace Common_Robot2
{
    ...
    public class C_Robot_DaZu
    {
        ...
        public double db_a6_min = -250 - 90;
        public double db_a6_max = -250 + 90;

        public double db_a6_minus_a1 = 140;
        ...
        public void reset_angle() //float tools_len=183.28f)
        {
            ...
            rotate_center[2] = new Vector3(0, 0 - this.arm_od, arm_dz + arm_dc);// 1035.7f);
            obj_rotate[2] = new Vector3(0, -1, 0);//L3旋转的向量

            rotate_center[3] = new Vector3(0, 0 - this.arm_od, arm_dz + arm_dc + arm_ck);// 1827.2f);// -0.575f);
            obj_rotate[3] = new Vector3(0, -1, 0);//L4旋转的向量

            rotate_center[4] = new Vector3(0, 0 - this.arm_od, arm_dz + arm_dc + arm_ck + arm_kb);//  1985.7f);
            ...
            rotate_center[6] = new Vector3(0, 0 - this.arm_od - arm_ab - tools_len, arm_dz + arm_dc + arm_ck + arm_kb + 100);
        ...
        public List<string> 根据坐标计算6轴角度(C_Node pNode,C_Point3D A_new, C_Point3D B_new,C_Point3D P7)
        {
            ...
            if (bDebug) Main.WriteLine(pNode, "A=" + A_new);
            if (bDebug) Main.WriteLine(pNode, "B=" + B_new);

            (C_Point3D D, C_Point3D D_b) = Main_Robot.计算D坐标_Dazu(B_new, Dz, r1);//,r2);
            ...
            C_Point3D? G = Main_Robot.计算G坐标_Dazu(A_new, D);
            C_Point3D? G_b = Main_Robot.计算G坐标_Dazu(A_new, D_b);
            (C_Point3D K1, C_Point3D K2) = 计算K坐标_Dazu(A_new, B_new, D, G);
            (C_Point3D K3, C_Point3D K4) = 计算K坐标_Dazu(A_new, B_new, D_b, G_b);


            C_Point3D O = new C_Point3D(0, 0, h);//O不是原点 !! 调试用
            C_Point3D DK1 = K1 - D;
            C_Point3D DO = O - D;
            float cos = DO * DK1; //调试验证用！！
            ...
            //(C, Vx, Vy, Vz) = Main_Robot.计算C坐标_Dazu(D, K2, h, DC, CK);
            //if (C != null)
            //{
            //    calculate_save(pNode, list, A_new, B_new, D, G, C[0], Vx, K2, P7, h);
            //    calculate_save(pNode, list, A_new, B_new, D, G, C[1], Vx, K2, P7, h);
            //}

            (C, Vx, Vy, Vz) = Main_Robot.计算C坐标_Dazu(D_b, K3, h, DC, CK);
            if (C != null)
            {
                calculate_save(pNode, list, A_new, B_new, D_b, G_b, C[0], Vx, K3, P7, h);
                calculate_save(pNode, list, A_new, B_new, D_b, G_b, C[1], Vx, K3, P7, h);
            }

            (C, Vx, Vy, Vz) = Main_Robot.计算C坐标_Dazu(D_b, K4, h, DC, CK);
            if (C != null)
            {
                calculate_save(pNode, list, A_new, B_new, D_b, G_b, C[0], Vx, K4, P7, h);
                calculate_save(pNode, list, A_new, B_new, D_b, G_b, C[1], Vx, K4, P7, h);
            }
        ...
        public string calculate_save(
            ...
            C_Point3D D, C_Point3D G, C_Point3D C1,
            C_Point3D Vx, C_Point3D K,C_Point3D P7,float h)
        {
            (double a1, double a2, double a3, double a4, double a5)
                = calculate(pNode,A_new, B_new, D, G, C1, Vx, K, h);
            ...
            float db_a5 = Tools.rad_to_degree(a5);
            ...
            while (angles1[5] < this.db_a6_min)
            {
                angles1[5] += 360;
            }
            while (angles1[5] > this.db_a6_max)
            {
                angles1[5] -= 360;
            }
            angles1[5] += (float)this.db_diff;


            C_Point3D O = new C_Point3D(0, 0, h);//O不是原点 !! 调试用
            C_Point3D DK = K - D;
            C_Point3D DO = O - D;
            float cos= DO* DK; //调试验证用！！
            ...
            string line = string.Join(",",angles1);// db_a1 + "," + db_a2 + "," + db_a3 + "," + db_a4 + "," + db_a5 + "," + db_a6;

            //Main.WriteLine(pNode, "six=" + line);
        ...
        public (double a1, double a2, double a3, double a4, double a5)
            calculate(C_Node pNode,
            C_Point3D A_new, C_Point3D B_new, C_Point3D D, C_Point3D G,
            C_Point3D C, C_Point3D Vx, C_Point3D K, double h = 185.7)
        {
            ...
            C_Point3D? vector_ck = (K - C).normalize();
            double a3 = Math.PI - Main.my_acos(vector_cd * vector_ck);
            if (vector_cd.crossProduct(vector_ck) * OD > 0)
            {
                a3 = -a3;
            }
            a3 = angle_process(a3);
            ...
            C_Point3D vector_kc = (C - K).normalize();
            C_Point3D vector_kb = (K - B_new).normalize();
            double a4 = Main.my_acos(vector_kc * vector_kb);
            if (vector_kc.crossProduct(vector_kb) * OD < 0)
            {
                a4 = -a4;
            }
            a4 -= Math.PI / 2;
            ...
            double a5;
            C_Point3D AG = G - A_new;
            C_Point3D BG = G - B_new;



            if (AG.length() < 0.001) //A在DCK平面上
            {
                C_Point3D vector_ba = (A_new - B_new).normalize();
                C_Point3D vector_bg = (G - B_new).normalize();

                // vector_bg 做为x
                C_Point3D v_y = vector_bg.normalize();
                C_Point3D v_x = vector_bg.crossProduct(vector_kb * (-1)).normalize();

                a5 = Math.Atan2(vector_ba * v_y, vector_ba * v_x);
                a5 = a5 - Math.PI;
            ...
            else if (BG.length() < 0.001)  //AB垂直DCK平面，这个会有多个解
            {

                //OD,AB如果是同方向的就是180度，否则就是0度
                if ((D - O) * (B_new - A_new) > 0)
                {
                    a5 = Math.PI;
                }
                else
                ...
                //C_Point3D vector_ba = (A_new - B_new).normalize();
                //C_Point3D vector_bg = vector_ba.crossProduct(vector_kb * (-1)).normalize();
            ...
            else
            {
                ...
                // vector_bg 做为x
                C_Point3D v_y = vector_bg.normalize();
                C_Point3D v_x = v_y.crossProduct(vector_kb * (-1)).normalize();

                if (v_x * DO < 0)
                {
                    v_y = (v_y * (-1)).normalize();
                    v_x = v_y.crossProduct(vector_kb * (-1)).normalize();
                }
            ...
            a5 = angle_process(a5);
            //Main.WriteLine(this, "5=" + a5 + ",=" + Tools.rad_to_degree(a5));
            ...
            float k_up = 100, float k_down = 100)
        {
            ...
            List<string> list_angle = this.根据坐标计算6轴角度(pNode, n3.A, n3.B,n3.P7);
            ...
            (C_Point3D A_, C_Point3D B_, C_Point3D K, C_Point3D C, C_Point3D D, C_Point3D P1) = this.根据角度计算ABC2CD(angles, len_ab);
        ...
        /// <returns></returns>
        public (C_Point3D A, C_Point3D B, C_Point3D K, C_Point3D C, C_Point3D D, C_Point3D P7)
            ...
            求正解_角度(float[] angle, float distance_AB)
        {
            ...
            Vector3 vA = this.rotate_center[5];
            C_Point3D A = new C_Point3D(vA);

            Vector3 vB = this.rotate_center[4];
            C_Point3D B = new C_Point3D(vB);

            Vector3 v7 = this.rotate_center[6];
