...
namespace Common_Robot2
{
    public class C_Space
    {
        ...
        public void Log_Debug_Thread_Start()
        {
            ...
            {
                Random p=new Random();
                string time_id = DateTime.Now.ToString("yyyy_MM_dd__HH_mm_ss__FFF_")+p.Next(10000);

                this.file_log = this.Name + "_" + this.tag + "_" + time_id + ".txt";
        ...
        public string read_string(C_Node pNode,string? line_string,string strDefault="")
        {
            string line= read_string_sub(pTrain, pNode, line_string);
            if (line == "")
            {
                line = strDefault;
            }
        ...
        public string read_string_sub(I_Train? pTrain, C_Node pNode, string? value)
        {
            string? value2 = value;
            ...
            {
                string var_name = value2.Substring(1);

                object? obj = this.read_vars(pTrain,pNode, var_name);
                if (obj == null) return "";
        ...
        public void save_vars(I_Train? pTrain, C_Node pNode_Debug, string? key, string str_type, Object? value)
        {
            if (key == "#p7_dx" || key == "#p7_dy")
            {
                Console.WriteLine("D");
            }
            ...
            if (key == "" || key == null)
            {
                Main.WriteLine(pNode_Debug,"key==null");//);
                return;
            }
            try
            {
                if (key.StartsWith("&"))
                {
                    ...
                    if (key2.StartsWith("#"))
                    {
                        if (this.vars_new.ContainsKey(key2))
                        {
                            ...
                            {
                                container = (C_Container?)this.vars_new[key2].get_obj();
                            }
                            else
                            {
                    ...
                    else
                    {
                        if (pTrain!=null && pTrain.get_Vars().ContainsKey(key2))
                        {
                            ...
                            {
                                container = (C_Container?)pTrain.get_Vars()[key2].get_obj();
                            }
                            else
                            {
                                MessageBox.Show(key2 + "这个变量不是容器");
                ...
                {
                    C_Data? pData = (C_Data?)pTrain?.get_Vars()["main"].get_obj();
        ...
        /// <returns></returns>
        public object? read_vars(I_Train? pTrain, C_Node? pNode, string key, string str_type="string")
        {
            if (key == "#图片1")
            {
                Console.WriteLine("D");
        ...
        public void Log_Thread_Start(Thread? pThread)
        {
            ...
            pThread = new Thread(() =>
            {
                string time2 = DateTime.Now.ToString("yyyy_MM_dd__HH_mm_ss_fff");
                string path = Application.StartupPath + "\\logs\\";
                if (Directory.Exists(path) == false)
                {
                    Directory.CreateDirectory(path);
                }
                string File_Log = Application.StartupPath + "\\logs\\space_" + this.Name + "_" + this.tag + "_" + time2 + ".txt";
                Action<string> writeLog = (message) =>
                {
                    lock (_lock_logs)
                    {
            ...
            while (this.vars.bClosingWindows == false)
            {
                ...
                lock (this.actives_Nodes)
                {
                    ...
                    for(var i = 0; i < this.actives_Nodes.Count; i++)
                    {
                        ...
                        if (pNode.mode != null && pNode.mode != "" &&  pNode.mode != "0")
                        {
        ...
        public void copy_from(C_Space space2,C_Node pNode)
        {

            CommonMain.WriteLine(this.ID + " copy_from " + space2.ID);

            this.vars_new = space2.vars_new;

            //foreach (var item in space2.vars_new)
            //{
            //    if (item.Value.name.StartsWith("#"))
            //    {
            //        this.save_vars(null, pNode, item.Value.name, "", item.Value.pObj);
            //    }
            //}
